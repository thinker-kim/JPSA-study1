...
OUTPUT_FILE = BASE_DIR / "rq2analticsample_with_google_scholar_metadata.csv"
CACHE_DB_FILE = BASE_DIR / "google_scholar_full_cache.sqlite"
ERROR_LOG_FILE = BASE_DIR / "google_scholar_full_errors.log"
...
def main() -> None:
    ...
    print("=" * 80)
    print("Google Scholar 전체 데이터 수집")
    print("=" * 80)
...
# ============================================================
# 전체 실행 최적화 + 기존 결과 이어서 실행
# ============================================================

# Scholar 검색 비용 절감
RESULTS_PER_PAGE = 10
MAX_QUERIES_PER_PAPER = 2
MAX_TITLE_VARIANTS = 1
ENABLE_AUTHOR_REFINED_SEARCH = True
ENABLE_SELECTIVE_SECOND_PAGE = False

# 원문 검증 속도 개선
ACCESS_REQUEST_TIMEOUT_SECONDS = 8
ACCESS_MAX_DOWNLOAD_CANDIDATES = 2


def build_title_variants(row: pd.Series) -> list[dict[str, str]]:
    """
    논문당 가장 신뢰할 수 있는 제목 하나만 선택한다.
    ...
    기존처럼 영어/한국어 제목을 모두 별도 검색하지 않는다.
    """
    candidates = [
        {
            "title": clean_text(row.get("kci_title_original")),
...
def _best_candidate(
    ...
    return max(
        ...
        key=lambda x: (
            ...
            x["corroboration_count"],
            -(
                x["global_result_position"]
                if x["global_result_position"] is not None
                else 9999
            ),
        ),
    )
...
def _needs_author_refined_search(
    variant: dict[str, str],
    best: Optional[dict[str, Any]],
) -> bool:
    """
    두 번째 SerpAPI 쿼리는 첫 검색이 실제로 애매할 때만 사용한다.

    검색결과가 전혀 없거나 명백히 다른 결과뿐이면 추가 검색하지 않는다.
    제목이 짧거나, 제목 유사도가 어느 정도 있으나 accept 기준에는
    미달한 경우에만 저자 제한 검색을 수행한다.
    """
    if not clean_text(variant.get("author")):
        return False

    if best is None:
        return False

    status, _, accepted = classify(best)
    if accepted:
        return False

    title_words = len(normalize_title(variant["title"]).split())
    title_score = float(best.get("title_score") or 0.0)
    weighted_score = float(best.get("weighted_score") or 0.0)

    plausible_but_ambiguous = (
        status == "review"
        or title_score >= REVIEW_TITLE_SCORE
        or weighted_score >= REVIEW_WEIGHTED_SCORE
    )

    short_title_with_some_signal = (
        title_words <= 7
        and title_score >= 60.0
    )

    return plausible_but_ambiguous or short_title_with_some_signal
...
def process_one_paper(
    ...
    """
    기본 1쿼리, 예외적으로만 저자 제한 2쿼리.

    두 번째 페이지 검색과 여러 제목 변형 검색은 하지 않는다.
    """
    variants = build_title_variants(row)
    if not variants:
        return blank_result("missing_search_title")
    ...
    network_queries += int(network_used)
    cache_hits += int(not network_used)
    last_error = clean_text(response.get("api_error"))
    ...
    # --------------------------------------------------------
    # 2차: 첫 검색이 애매한 경우에만 저자 제한 검색
    # --------------------------------------------------------
    if (
        ENABLE_AUTHOR_REFINED_SEARCH
        and network_queries < MAX_QUERIES_PER_PAPER
        and _needs_author_refined_search(variant, best)
    ):
        refined_query = author_refined_query(
            variant["title"],
            variant["author"],
        )

        if refined_query and refined_query != query:
            refined_response, refined_network_used = serpapi_search(
                conn,
                session,
                refined_query,
                0,
            )
            network_queries += int(refined_network_used)
            cache_hits += int(not refined_network_used)
            last_error = (
                clean_text(refined_response.get("api_error"))
                or last_error
            )

            refined_ranked = rank_results(
                row,
                variant,
                refined_query,
                0,
            ...
            all_candidates.extend(refined_ranked)
            best = _best_candidate(all_candidates)
...
def completed_uids_from_output() -> set[str]:
    """
    기존 출력 CSV에 이미 저장된 UID를 읽는다.

    따라서 프로그램을 다시 실행해도 처음부터 시작하지 않고,
    아직 저장되지 않은 논문만 처리한다.
    """
    if not OUTPUT_FILE.exists() or OUTPUT_FILE.stat().st_size == 0:
        return set()
...
def main() -> None:
    """
    기존 출력 파일의 완료 UID를 자동으로 건너뛰고 이어서 실행한다.
    """
    if SERPAPI_KEY == "123":
        print(
            "주의: SERPAPI_KEY가 예시값 '123'입니다. "
            "실제 키로 교체하세요."
        )
    ...
    if df["paper_uid_after_direct_w"].duplicated().any():
        raise ValueError(
            "paper_uid_after_direct_w에 중복값이 있습니다."
        )

    completed = completed_uids_from_output()

    uid_series = (
        df["paper_uid_after_direct_w"]
        .fillna("")
        .astype(str)
        .str.strip()
    )
    ...
    rows = df.loc[
        uid_series.ne("")
        & ~uid_series.isin(completed)
    ].copy()

    print("=" * 80)
    print("Google Scholar 전체 데이터 수집 — 최적화·이어하기 버전")
    print("=" * 80)
    print(f"입력 파일: {INPUT_FILE}")
    print(f"출력 파일: {OUTPUT_FILE}")
    print(f"전체 대상: {len(df):,}")
    print(f"기존 완료 UID: {len(completed):,}")
    print(f"이번 실행에서 남은 UID: {len(rows):,}")
    print("검색: 기본 1쿼리, 애매한 경우에만 최대 2쿼리")
    print("두 번째 페이지 검색: 사용 안 함")
    print("원문 검증 timeout: 8초")
    print("다운로드 후보 확인: 최대 2개")
    print()

    if rows.empty:
        print("처리할 새 논문이 없습니다.")
        return
    ...
    session.headers.update(
        {
            "User-Agent": (
                "Mozilla/5.0 "
                "GoogleScholarMetadataResearch/1.0"
            ),
            "Accept": "application/json",
        }
    )
    ...
    processed = 0
    network_query_total = 0
    cache_hit_total = 0

    try:
        for _, row in rows.iterrows():
            uid = clean_text(
                row["paper_uid_after_direct_w"]
            )
            if not uid:
                continue
            ...
            # 논문 한 편이 끝날 때마다 저장:
            # 중단 후 재실행하면 이 UID부터 자동으로 건너뛴다.
            append_row_immediately(row, result)

            processed += 1
            network_query_total += int(
                result.get(
                    "google_scholar_query_count_used"
                )
                or 0
            )
            cache_hit_total += int(
                result.get(
                    "google_scholar_local_cache_hits"
                )
                or 0
            )
            ...
            print(
                ...
                f"score={result.get('google_scholar_match_score')} | "
                f"queries={result.get('google_scholar_query_count_used')} | "
                f"cache={result.get('google_scholar_local_cache_hits')} | "
                f"citations={result.get('google_scholar_citation_count')} | "
                f"access={result.get('google_scholar_fulltext_access')}"
            )

    except KeyboardInterrupt:
        print(
            "\n중단되었습니다. 이미 저장된 UID는 다음 실행에서 "
            "자동으로 건너뜁니다."
        )
    ...
    print()
    print(f"이번 실행 저장 논문 수: {processed:,}")
    print(f"이번 실행 네트워크 Scholar 쿼리: {network_query_total:,}")
    print(f"이번 실행 로컬 캐시 사용: {cache_hit_total:,}")
    print(f"결과 파일: {OUTPUT_FILE}")
...
# 검색 회수율 보완 v2
# - 최대 2쿼리 유지
# - 1차 원문 제목 검색 실패/저점수 시 영어 제목 fallback
# - 영어 제목이 없을 때만 저자 제한 검색
# ============================================================

def build_search_variants(row: pd.Series) -> list[dict[str, str]]:
    """
    최대 두 개의 서로 다른 제목 후보를 준비한다.
...
def _is_very_weak_candidate(
    best: Optional[dict[str, Any]],
) -> bool:
    """
    1차 검색 결과가 없거나 사실상 관련성이 매우 낮으면 True.

    이 경우 저자 제한 검색보다 다른 언어 제목 검색을 우선한다.
    """
    if best is None:
        return True
    ...
    return (
        title_score < REVIEW_TITLE_SCORE
        and weighted_score < REVIEW_WEIGHTED_SCORE
    )
...
def process_one_paper(
    ...
    """
    최대 2개의 SerpAPI 쿼리만 사용한다.

    검색 순서:
      1. KCI 원문 제목(없으면 영어/참조 제목)
      2-a. 1차 결과 없음 또는 매우 약함:
           다른 언어/대체 제목 검색
      2-b. 1차 결과는 어느 정도 유사하지만 애매함:
           같은 제목 + 저자 제한 검색

    따라서 한국어 제목으로 검색되지 않지만 영어 제목으로 색인된
    논문을 보완하면서도 논문당 최대 2쿼리를 유지한다.
    """
    variants = build_search_variants(row)
    if not variants:
        return blank_result("missing_search_title")
    ...
    primary_query = exact_title_query(primary_variant["title"])
    ...
    # --------------------------------------------------------
    if network_queries < MAX_QUERIES_PER_PAPER:
        second_variant: Optional[dict[str, str]] = None
        second_query = ""

        # 1차 결과가 없거나 매우 약하면 영어/대체 제목 fallback
        if _is_very_weak_candidate(best) and len(variants) >= 2:
            second_variant = variants[1]
            second_query = exact_title_query(second_variant["title"])

        # 1차 결과가 어느 정도 유사하지만 애매하면 저자 제한 검색
        elif _needs_author_refined_search(primary_variant, best):
            second_variant = primary_variant
            second_query = author_refined_query(
                primary_variant["title"],
                primary_variant["author"],
            )

        if (
            second_variant is not None
            and second_query
            and second_query != primary_query
        ):
            second_response, second_network_used = serpapi_search(
                conn,
                session,
                second_query,
                0,
            )
            network_queries += int(second_network_used)
            cache_hits += int(not second_network_used)
            last_error = (
                clean_text(second_response.get("api_error"))
                or last_error
            )

            second_ranked = rank_results(
                row,
                second_variant,
                second_query,
                0,
            ...
            all_candidates.extend(second_ranked)
            best = _best_candidate(all_candidates)
